...
namespace InventoryManagement.Controllers
{
    public class InvoiceController : Controller
    {
        ...
        public async Task<IActionResult> Create(InvoiceViewModel model)
        {
            ...
            // Build invoice items with real product prices
        ...
        {
            _logger.LogInformation("Edit POST action started for Invoice ID: {InvoiceId}", model.Id);

            if (!ModelState.IsValid)
            {
                _logger.LogWarning("ModelState is invalid for Invoice ID: {InvoiceId}", model.Id);
